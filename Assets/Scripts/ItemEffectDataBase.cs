...
public class ItemEffect
{
    ...
    [Tooltip("HP,DP,SP,HUNGRY,THIRSTY,SATISFY 만 가능합니다.")]
...
public class ItemEffectDataBase : MonoBehaviour
{
    ...
    public void UseItem(Item _item)
    {
        if (_item.itemType == Item.ItemType.Equipment)
            StartCoroutine(theWeaponManager.ChangeWeaponCoroutin(_item.weaponType, _item.itemName));
                    ...
                    for (int y = 0; y < itemEffects[x].part.Length; y++)
                    {
                         switch(itemEffects[x].part[y])
                        {
                            ...
                            case SATISFY:
                                break;
                            default:
                                Debug.Log("잘못된 Status 부위. HP,DP,SP,HUNGRY,THIRSTY,SATISFY 만 가능합니다.");
                                break;
                        }
